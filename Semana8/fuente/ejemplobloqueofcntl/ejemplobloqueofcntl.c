...
int main(void)
{
	...
	checabloqueo(descriptorarchivo,1L,F_WRLCK,10L,"Sección A");
	...
	if(!fork())
	{
		...
		checabloqueo(descriptorarchivo,2L,F_WRLCK,10L,"Sección B");
		if(read(descriptorarchivo,buffer,TAM)<0)
		...
		lseek(descriptorarchivo,0L,0);
		checabloqueo(descriptorarchivo,1L,F_WRLCK,10L,"Sección C");
		if(read(descriptorarchivo,buffer,TAM)<0)
	...
	else
	{
		...
		lseek(descriptorarchivo,10L,0);
		checabloqueo(descriptorarchivo,2L,F_WRLCK,10L,"Sección D");
		if(read(descriptorarchivo,buffer,TAM)<0)
